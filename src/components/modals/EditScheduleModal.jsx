...
const EditScheduleModal = ({ show, onClose, onSave, scheduleDate, initialScheduleEntries, allEmployees, positions }) => {
  ...
  useEffect(() => {
    if (show && initialScheduleEntries) {
      ...
      initialScheduleEntries.forEach(entry => { Object.keys(entry).forEach(key => { if(!['scheduleId', 'empId', 'date', 'name', 'shift'].includes(key)) { existingColumns.add(key); } }); });
                    ...
                    <tr>
                      ...
                      {columns.map(col => (
                        <th key={col.key} className="text-center custom-column">
                          {col.name}
                          {col.key !== 'shift' && (<button type="button" className="btn btn-sm btn-outline-danger p-0 ms-2 delete-column-btn" onClick={() => handleDeleteColumn(col.key)} title={`Delete '${col.name}' column`}><i className="bi bi-x"></i></button>)}
                        </th>
                      ))}
