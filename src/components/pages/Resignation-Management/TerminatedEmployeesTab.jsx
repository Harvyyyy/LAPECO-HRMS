...
const TerminatedEmployeesTab = ({ employees, terminations, positions, positionMap, payrolls, onRehire, onDelete, onViewProfile, onViewFinalPay }) => {
    ...
    const terminatedEmployeesWithData = useMemo(() => {
        return employees
            ...
            .map(emp => {
                const positionObject = positions.find(p => p.id === emp.positionId);
    ...
    const filteredAndSorted = useMemo(() => {
        ...
        return [...filtered].sort((a, b) => {
            ...
            if (key === 'terminationDate') {
                const termA = terminations.find(t => t.employeeId === a.id);
                const termB = terminations.find(t => t.employeeId === b.id);
                valA = termA?.date || '';
                valB = termB?.date || '';
            }
                ...
                <table className="table data-table mb-0 align-middle">
                    ...
                    <tbody>
                        {filteredAndSorted.length > 0 ? filteredAndSorted.map(emp => {
                            const terminationRecord = terminations.find(t => t.employeeId === emp.id);
                            return (
                                <tr key={emp.id}>
                                    <td>{emp.id}</td>
                                    <td><div className="fw-bold">{emp.name}</div></td>
                                    <td>{positionMap.get(emp.positionId) || 'Unassigned'}</td>
                                    <td>{terminationRecord?.date || 'N/A'}</td>
