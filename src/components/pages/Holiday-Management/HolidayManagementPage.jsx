...
const HolidayManagementPage = ({ holidays, leaveRequests, handlers }) => {
  ...
  useEffect(() => {
    if (viewMode === 'calendar' && calendarRef.current) {
      ...
      const newDate = `${filterYear}-01-01`;
  ...
  const filteredHolidays = useMemo(() => {
    ...
    return holidaysForYear
      .filter(h => 
        ...
        h.name.toLowerCase().includes(lowerSearchTerm)
      )
  ...
  const calendarEvents = useMemo(() => {
    const holidayEvents = filteredHolidays.map(h => ({
      ...
      date: h.date,
      allDay: true,
      extendedProps: { type: h.type, icon: h.type === 'Regular Holiday' ? 'bi-bookmark-star-fill' : 'bi-star-fill' },
      className: h.type === 'Regular Holiday' ? 'fc-event type-regular' : 'fc-event type-special',
    }));
  ...
  const stats = useMemo(() => {
    ...
    const regular = holidaysForYear.filter(h => h.type === 'Regular Holiday').length;
  ...
  const handleSaveHoliday = (formData, holidayId) => { handlers.saveHoliday(formData, holidayId); handleCloseModal(); };
  const handleDeleteHoliday = (e, holidayId) => { handlers.deleteHoliday(holidayId); };
